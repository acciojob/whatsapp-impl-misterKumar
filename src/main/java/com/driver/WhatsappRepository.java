...
public class WhatsappRepository {
    ...
    public String changeAdmin(User approver,User user,Group group)throws Exception{
        ...
        if(!this.userExists(group,user))throw new Exception("User is not a participant");
        adminMap.put(group,user);
        return "SUCCESS";
    }


    private boolean userExists(Group group, User user) {
    ...
    public  int sendMessage(Message message,User user,Group group)throws Exception{
        if(!groupUserMap.containsKey(group))throw new Exception("Group does not exist");
        if(!this.userExists(group,user))throw new Exception("You are not allowed to send message");
